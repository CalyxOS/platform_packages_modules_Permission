...
class PrivacyControlsViewModel(private val app: Application) : AndroidViewModel(app) {
    ...
    fun setClipboardAutoClearTimeout(timeout: Long): Boolean {
        return setNameValuePair(
            app.contentResolver,
            LINEAGE_SETTINGS_SECURE_URI,
            LINEAGE_SETTINGS_SECURE_CLIPBOARD_AUTO_CLEAR_TIMEOUT,
            timeout.toString())
    }

    /**
     * Insert a single name/value pair. Assumes the content provider uses "ON CONFLICT REPLACE"
     * for inserts with the same name column. Returns whether the operation was successful.
     */
    private fun setNameValuePair(
        ...
        try {
            return contentResolver.insert(uri, values) != null
        } catch (e: Exception) {
            Log.e(TAG, "Failed to write $values to $uri", e)
        }
        return false
    }
}
